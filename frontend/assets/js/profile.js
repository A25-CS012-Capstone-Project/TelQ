        ...
        <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 ${borderClass} hover:shadow-md transition flex justify-between items-center group mr-1">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="${tagClass} text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide">${item.category}</span>
                </div>
                <h4 class="font-bold text-gray-800 text-sm group-hover:text-blue-600 transition">${item.product_name}</h4>
                <p class="text-xs text-gray-500 mt-1">
                    Data: <b>${item.data_gb} GB</b> | Validity: ${item.validity_days} Hari
                </p>
            </div>
            <div class="text-right">
                <p class="font-bold text-gray-800 text-sm">${fmtMoney.format(item.price)}</p>
